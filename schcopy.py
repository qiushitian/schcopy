...
# FITS_PATH = r"D:\chris-tian\copy-folder-test"
...
COPY_PATH = r"\\tsclient\mountpoint\space-raw"
# COPY_PATH = r"\\tsclient\mountpoint\copy-folder-test"

# path for robocopy logs
LOG_PATH = r"D:\chris-tian\copy-folder-log"
...
if __name__ == '__main__':
    ...
    with os.scandir(FITS_PATH) as date_iterator:
        for date_entry in date_iterator:
            # skip non-folders
            ...
            with os.scandir(date_entry.path) as target_iterator:
                for target_entry in target_iterator:
                    # skip non-folders
                    ...
                    # make dir
                    mdcmd = ['md', os.path.join(COPY_PATH, date)]
                    ...
                    robocmd = ['robocopy',
                               target_path,
                               os.path.join(COPY_PATH, date, target_dash),
...
##### FROM chriscessor.py ####
    ...
    #             new_file_path = os.path.join(COPY_PATH, date_folder_name, filename)
    #
    #             # Copy the file to the COPY_PATH directory
    ...
    #     csv_path = os.path.join(COPY_PATH, f"{object_name.replace('/', '-')}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv")
    #
    #     # Write the CSV file
